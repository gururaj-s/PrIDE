...
sub get_cstats{
    ...
    ## PARA num_rfms * 4 / (dram_reads + dram_writes)
    $num1_statname = "rh_num_rfms";
    $den1_statname = "dram.num_reads::total";
    $den2_statname = "dram.num_writes::total";
    $num_mult = 4;
    ...
    for($dirnum=0; $dirnum<@dirs; $dirnum++){
        for($ii=0; $ii< $num_w; $ii++){
            $temp_ndata[$dirnum][$ii] = $ndata[$dirnum][$ii] * $num_mult;
        }
    }
    $nstat = "";
    ...
    for($dirnum=0; $dirnum<@dirs; $dirnum++){
        for($ii=0; $ii< $num_w; $ii++){
            $temp_ddata[$dirnum][$ii] = $ddata[$dirnum][$ii] ;
        }
    }
    ...
    for($dirnum=0; $dirnum<@dirs; $dirnum++){
        for($ii=0; $ii< $num_w; $ii++){
            $data[$dirnum][$ii] = 0 ;
            $data[$dirnum][$ii] = $temp_ndata[$dirnum][$ii]  / $temp_ddata[$dirnum][$ii]  if($temp_ddata[$dirnum][$ii]) ;
        }
    }
