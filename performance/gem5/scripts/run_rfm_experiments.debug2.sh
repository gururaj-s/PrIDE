...
; do 
    ...
    if [ $qsub -gt 0 ] 
    then
	echo "./runscript.debug.sh $bmk $config 4 2017 --rh_defense --rh_mitigation=${defense_name} --rh_rfm_per_trefi=${rfm} --rh_rfm_blast_radius=${br} --rh_rfm_maxacts_per_trefi=${max_act_trefi}" >> $qsub_cmdfile;
    else    	    
#    ./runscript.debug.sh $bmk $config 4 2017 \
    ./runscript.debug2.sh $bmk $config 4 2017 \
