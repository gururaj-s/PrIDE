...
void Randomized_Row_Swap::resetDecision() {
    ...
    decision.bus_delay = 0;
    decision.bank_delay = 0;
    for (int i = 0; i < 6; i++) {
...
void Row_Quarantine::drainRow() {
    ...
    printf("[RQ] DRAIN function- HEAD: %d, PREV-HEAD: %d, TAIL: %d VALID? %d\n",
...
RFM::RFM(std::string __rh_mitigation, uint32_t __threshold, bool __bus_delay, uint32_t __RFMThreshold, uint32_t __delayTicksTRC, uint32_t __delayTicksTRFC, uint32_t __blastRadius,
         uint32_t __numRows, uint8_t __numBanks, uint8_t __numRanks, uint8_t __numChannels, uint32_t tracker_size) {
    ...
    rh_mitigation = __rh_mitigation;
    bus_delay = __bus_delay;
    threshold = __threshold;
    rfm_act_threshold = __RFMThreshold;
    delay_ticks_tRC = __delayTicksTRC;
    delay_ticks_tRFC = __delayTicksTRFC;
    blast_radius = __blastRadius;
    numRows = __numRows;    
    numBanks = __numBanks;
    numRanks = __numRanks;
    numChannels = __numChannels;
    ...
    // initialize RFM act counters.
    uint32_t numDRAMBanks = numBanks*numRanks*numChannels;
    // printf("numChannels: %d, numRanks: %d, numBanks: %d. DRAMBanks: %d.\n",numChannels, numRanks,numBanks,numDRAMBanks);

    mytracker = new Tracker[numDRAMBanks];
    for(int i=0; i<numDRAMBanks; i++)
    {
        mytracker[i].Init(tracker_size);
    }

    sample_rate = 1 / (double) (rfm_act_threshold+1);
    
//     printf("RFM details - rfm_act_threshold: %ld, blast_radius: %ld, delay_per_rfm: %ld ps. sample_rate: %f\n",rfm_act_threshold, blast_radius,delay_ticks_tRC * 2 * blast_radius, sample_rate);
    printf("RFM details - rfm_act_threshold: %ld, blast_radius: %ld, delay_per_rfm: %ld ps. sample_rate: %f\n",rfm_act_threshold, blast_radius, delay_ticks_tRFC, sample_rate);

    act_in_trefi_per_bank.resize(numDRAMBanks);
    for(int i=0; i< numDRAMBanks ; i++){
      act_in_trefi_per_bank[i] = 0; 
    }
    
    // initialize logging stats
    for (int i=0; i<= 8 ; i++){
      s_numLogBankAccesses[i]= 0;
    }
    s_numBankAccesses_RFMThreshold = 0;
    resetDecision();
}

Decision& RFM::access(Tick currTick, Addr rowID, uint8_t bankID, uint8_t rankID, uint8_t channelID) {
    s_num_accesses++;
    resetDecision();

    // Make sure ckpt restore does not mess with last_reset tick. 
    if(last_reset == 0)
      last_reset = currTick;

    // increment act counters.
    uint64_t currBankID = channelID * numRanks * numBanks + rankID * numBanks + bankID;
    assert( ( currBankID < act_in_trefi_per_bank.size()) && "BankID not within NumBanks\n");

    // Check if reset of the counters is needed
    if (rh_mitigation == "RFM"){
//      if (currTick - last_reset >= 7800000) {
      if (currTick - last_reset >= 3900000) {
        last_reset = currTick;
        reset();
        
        // Issue mitig?
        for( int i=0; i<numBanks*numRanks*numChannels; i++)
        {
            int send_mitig = mytracker[i].issue_mitig() != -1;
            if( send_mitig ) s_mitigs++;
        }
      }
    }

    act_in_trefi_per_bank[currBankID]++;

    // Issue mitig?
    // if act counters beyond the rfm threshold, invoke mitigation.
    if ( (act_in_trefi_per_bank[currBankID] % rfm_act_threshold == 0) ) {
      //decision.delay = delay_ticks_tRC * 2 * blast_radius;
      decision.delay = 0 ;
//       decision.bank_delay = delay_ticks_tRC * 2 * blast_radius;
      decision.bank_delay = delay_ticks_tRFC;
      decision.bus_delay = bus_delay ? 3332 * 2 * blast_radius : 0 ;
      decision.mitigate = true;

      int send_mitig = mytracker[currBankID].issue_mitig() != -1;
      if( send_mitig ) s_mitigs++; 
      s_rfms++;
    }
    ...
    double normalized_rand = static_cast<double>(rand_num) / RAND_MAX;
    if( normalized_rand < sample_rate )
    {
        mytracker[currBankID].insert(rowID);
    }
...
void RFM::reset() {
    ...
    // Log the per bank counter values.
    uint32_t numDRAMBanks = numBanks*numRanks*numChannels;
    for (int rval = 0; rval < numDRAMBanks; rval++) {
        // banks with accesses more than RFM threshold (atleast 1 mitigation)
        if(act_in_trefi_per_bank[rval] >= rfm_act_threshold)
          s_numBankAccesses_RFMThreshold++;
      
        int logval = 0;
        for (int i = 0; i <= 8; i++) {
          int next_logval = (logval==0)? 2 : 2*logval ;
          next_logval = (next_logval > 166)? 166: next_logval;
          if ((act_in_trefi_per_bank[rval] >= logval) && ( (act_in_trefi_per_bank[rval] < next_logval) || (i == 8) )) {
            s_numLogBankAccesses[i]++;
            break;
          }
          logval = next_logval;
        }
        
        // reset RFM act counters.
        act_in_trefi_per_bank[rval] = 0;
    }
}

void RFM::resetDecision() {
    decision.validAddrs = 0;
    decision.remappedAddr = -1;
    decision.mitigate = false;
    decision.delay = 0;
...
// TODO: RFM
// 8.b. Ensure RFM only delays subsequent access to same bank (bank delayed by trc *k, but other banks can be serviced meanwhile).
// ---> RFM + PRESS.
// - 1. RFM-CMD-ON-BUS:  -- NO DELAY for CMD. (decision.data_bus_delay)
// - 2. BANK BUSY K ACTS -- mem_interface.cc: bank busy for 2*tRC*br (passed to dram->doburst) && precharge bank. (decision.bank_delay)
//                       -- bank_ref.actAllowedAt incremented by bank_delay, bank_ref[others].actAllowedAt += tRR*numACTs.
// ---> PARA.
// - 1. K*ACT*CMD-ON_BUS + tCL + tBURST: mem_interface.cc: next_burst_at += K-ACT*(tBURST) delay  (decision.data_bus_delay)
// - 2. BANK BUSY K ACTS -- mem_interface.cc: bank busy for 2*tRC*br (passed to dram->doburst) && precharge bank. (decision.bank_delay)

// 8.c Check how  reset can be called on tREFIs (instead of every 7.8us)
