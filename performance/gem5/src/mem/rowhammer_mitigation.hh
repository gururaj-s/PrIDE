...
public:
    ...
    uint64_t            s_rfms;
    uint64_t            s_mitigs;
    uint64_t            s_numLogBankAccesses[9];
    uint64_t            s_numBankAccesses_RFMThreshold;
...
class RFM : public RH_Mitigation {
private:
    vector<uint64_t> act_in_trefi_per_bank;
    void resetDecision();
    Tracker *mytracker;    
    double  sample_rate;
...
public:
    bool bus_delay;
    uint64_t rfm_act_threshold;
    Tick delay_ticks_tRC;
    Tick delay_ticks_tRFC;
    uint64_t blast_radius; 
    std::string rh_mitigation; 

    RFM(std::string rh_mitigation, uint32_t threshold, bool bus_delay, uint32_t __rfm_threshold, uint32_t __delayTicksTRC, uint32_t __delayTicksTRFC, uint32_t __blastRadius,
        uint32_t __numRows, uint8_t __numBanks, uint8_t __numRanks, uint8_t __numChannels, uint32_t tracker_size=6);
    Decision& access(Tick currTick, Addr rowID, uint8_t bankID, uint8_t rankID, uint8_t channelID);
    void reset();
